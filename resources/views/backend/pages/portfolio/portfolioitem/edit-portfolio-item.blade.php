                    ...
                    <div class="card-body">
                        <div class="border p-4 rounded">
                            <div class="card-title d-flex align-items-center">
                                ...
                                <h5 class="mb-0 text-info">Edit portfolio item</h5>
                            </div>
                            <hr />
                            <form action="{{ route('portfolio-item.update', $item->id) }}" method="post"
                                enctype="multipart/form-data">
                                @csrf
                                @method('PUT')

                                <div class="my-3">
                                    <label for="port_cat_id" class="form-label"> Portfolio Category</label>

                                    <select name="port_cat_id" id="port_cat_id" class="form-control single-select">
                                        <option value="" selected disabled>--select one category--</option>
                                        @foreach ($categories as $category)
                                            <option value="{{ $category->id }}"
                                                {{ $item->port_cat_id == $category->id ? 'selected' : '' }}>
                                ...
                                <div class="my-3">
                                  ...
                                  is-invalid
                                @enderror"
                                        placeholder="write your small details..">{{ $item->details }}</textarea>
                                    @error('details')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
